...
const reycilhook = new WebhookClient(client.veri.hosgeldinSistemi.webhookID, client.veri.hosgeldinSistemi.webhookTOKEN);
module.exports = {
    ...
    onRequest: async function (member) {
        let reycilkalkmazban = qDb.get(`akb_${member.guild.id}`)
        if(reycilkalkmazban && reycilkalkmazban.some(id => `k${member.user.id}` === id)) {
          try {
