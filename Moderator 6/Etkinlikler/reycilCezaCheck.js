...
module.exports = {
  ...
  function yasakKontrolEt() {
    ...
    for (let kisi of kalıcıcezalılar) {
        ...
        if (uye && !uye.roles.cache.has(reycil.Roller.jailRolu)) uye.roles.set(uye.roles.cache.has(reycil.Roller.boosterRolu) ? [reycil.Roller.boosterRolu, reycil.Roller.jailRolu] : [reycil.Roller.jailRolu]).catch();
      };
    for (let ceza of cezalılar) {
        ...
        if (Date.now() >= ceza.kalkmaZamani) {
          ...
          let erkeks = uye.roles.cache.filter(x => x.managed).map(x => x.id).concat(reycil.kayıtRolleri.erkekRolleri);
          let teyitsiz = uye.roles.cache.filter(x => x.managed).map(x => x.id).concat(reycil.kayıtRolleri.kayıtsızRolleri);
          let kizs = uye.roles.cache.filter(x => x.managed).map(x => x.id).concat(reycil.kayıtRolleri.kadinRolleri);
            ...
            for (let ceza of sürelisusturma) {
              let uye = client.guilds.cache.get(sid).members.cache.get(ceza.id);
                 if (Date.now() >= ceza.kalkmaZamani) {
                     ...
                     if (uye && uye.roles.cache.has(reycil.Roller.muteRolu)) uye.roles.remove(reycil.Roller.muteRolu).catch();
                     kullaniciverisi.set(`ceza.${ceza.No}.BitisZaman`, Date.now())
                } else {
                  if (uye && !uye.roles.cache.has(reycil.Roller.muteRolu)) uye.roles.add(reycil.Roller.muteRolu).catch();
                };
            };
